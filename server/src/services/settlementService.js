...
export const calculateBalances = (trip) => {
  const balances = Object.fromEntries(trip.members.map((member) => [normalizeId(member._id), 0]));
...
export const buildSettlements = (trip) => {
  ...
  const memberName = Object.fromEntries(trip.members.map((member) => [normalizeId(member._id), member.name]));
  const settlements = [];
  let i = 0;
  let j = 0;
